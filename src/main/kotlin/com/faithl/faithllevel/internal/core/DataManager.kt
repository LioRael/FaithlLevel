...
data class DataManager(val level: Level, val player: Player) {
    ...
    fun addExp(exp: Int): Boolean {
        ...
        if (playerLevel >= getMaxLevel()) {
            Level.getLevelFunc(level).event?.getConfigurationSection("player_level_max_warning.message")
                ?.sendMessage(player)
            return false
        }
    ...
    fun addLevel(level: Int): Boolean {
        if (playerLevel >= getMaxLevel()) {
            Level.getLevelFunc(this.level).event?.getConfigurationSection("player_level_max_warning.message")
                ?.sendMessage(player)
            return false
        }
        val event =
            LevelChangeEvent(this.level, player, playerLevel, playerLevel + level, LevelChangeEvent.ChangeType.ADD)
        event.call()
        if (event.isCancelled) {
            return false
        }
