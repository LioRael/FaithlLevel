...
internal object Event {
    ...
    fun e(e:ExpChangeEvent){
        if (e.type == ExpChangeEvent.ChangeType.ADD){
            Level.getLevelFunc(e.level).event?.getConfigurationSection("player_exp_up")?.run(e.player,Level.getPlayerData(e.level, e.player).playerLevel)
        } else if (e.type == ExpChangeEvent.ChangeType.TAKE){
            Level.getLevelFunc(e.level).event?.getConfigurationSection("player_exp_down")?.run(e.player,Level.getPlayerData(e.level, e.player).playerLevel)
        }
    }
}